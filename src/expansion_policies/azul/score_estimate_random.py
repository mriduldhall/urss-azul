...
class ScoreEstimateRandomExpansionPolicy:
    @staticmethod
    ...
    def choose_move(self, game, untried_moves, rng):
        scores = [self.score(game, move) for move in untried_moves]
        best_score = max(scores)

        best_moves = [
            move for move, score in zip(untried_moves, scores)
            if score == best_score
        ]
