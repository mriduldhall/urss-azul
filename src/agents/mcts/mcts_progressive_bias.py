...
class StateNode:
    ...
    def expand_node(self, legal_moves, rng, expansion_policy=None):
        ...
        if expansion_policy is not None:
            move = expansion_policy.choose_move(self.state, untried_moves, rng)
        else:
...
#noinspection DuplicatedCode
class MonteCarloTreeSearchProgressiveBiasAgent:
    def __init__(self, game, simulations=1000, exploration_constant=sqrt(2), rng=None, chance_policy=None, rollout_policy=None, expansion_policy=None, progressive_heuristic=None, k=1, alpha=0.5, lambda_bias=1):
        if simulations <= 0:
        ...
        self.progressive_heuristic = progressive_heuristic
        self.k = k
        self.alpha = alpha
    ...
    def select_child(self, current_node):
        ...
        for child in current_node.children:
            value = child.calculate_uct(self.exploration_constant, is_maximising)
            if self.progressive_heuristic is not None:
                child_is_maximising = child.state.current_player.value == self.game.current_player.value
                heuristic_value = self.progressive_heuristic.evaluate(child.state, child_is_maximising)
                heuristic_value = heuristic_value if is_maximising else -heuristic_value
                value += self.lambda_bias * (heuristic_value / (child.visits + 1))
    ...
    def find_node(self):
        ...
        while True:
            ...
            allowed_moves = min(len(legal_moves), floor(self.k * ((current_node.visits + 1) ** self.alpha)))

            if len(current_node.children) < allowed_moves:
                return current_node.expand_node(legal_moves, self.rng, self.expansion_policy)
