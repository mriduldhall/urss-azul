...
class StateNode:
    ...
    def expand_node(self, legal_moves):
...
class MonteCarloTreeSearchAgent:
    def __init__(self, game, simulations=1000, exploration_constant=sqrt(2)):
        self.game = game
    ...
    def find_node(self):
        ...
        while True:
            ...
            if len(current_node.children) < len(legal_moves):
                return current_node.expand_node(legal_moves)
            else:
    ...
    def run_simulation(self, state):
        ...
        while not cloned_state.check_end():
            move = choice(cloned_state.get_legal_actions())
            cloned_state.make_move(move)
        winner = cloned_state.check_victory()
        if winner is None:
            victory = None
        else:
            victory = winner == self.game.current_player
        return victory
